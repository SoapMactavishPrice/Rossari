...
export default class SearchableCombobox extends LightningElement {
	...
	fireChange() {
		console.log();
		this.dispatchEvent(new CustomEvent("change", { detail: { value: this.dataValue, Label: this.filtervalue } }));
	}
	...
	handleSelect(event) {
		...
		console.log('event.currentTarget.dataset.value ', event.currentTarget.dataset.label);
		this.isOpen = false;
		this.allowBlur();
	...
	handleKeyDown(event) {
		if (event.key == "Escape") {
			...
			if (this.highlightCounter !== null) {
				this.isOpen = false;
				this.allowBlur();
				console.log('handleKeyDown ', this.tempOptions[this.highlightCounter].label);
				this._value = this.tempOptions[this.highlightCounter].label;
				this.filtervalue = this.tempOptions[this.highlightCounter].label;
				this.dataValue = event.currentTarget.dataset.value;
				this.fireChange();
			}
