...
export default class FullfillmentPlanningExport extends NavigationMixin(LightningElement) {
    ...
    //         console.log('before');
    //         this.template.querySelector('.slds-line-height_reset').classList.add('mobiledata');
    //         console.log('after');
    //     }
    // }
    // renderedCallback() {
    ...
    //         if (FORM_FACTOR == 'Small') {
    //             console.log('before1');
    //             const elements = this.template.querySelectorAll('.slds-line-height_reset');
    //             elements.forEach(element => {
    //                 element.classList.add('mobiledata');
    //             });
    //             console.log('after1');
    //         }
    //     }, 7000);
    ...
    handleClick() {
        ...
        this.sbuFilter = '';
        this.distributionChannelFilter = '';
        this.divisionFilter = '';
        this.dispatchLocationCodeFilter = '';
        this.soStatusFilter = '';
        this.salesOrderNoFilter = '';
        this.soDateFilter = '';
        this.executiveFilter = '';
        this.customerCodeFilter = '';
        this.shipToPartyCodeFilter = '';
        this.regionFilter = '';
        this.cityFilter = '';
        this.materialGroup3Filter = '';
        this.skuCodeFilter = '';
        this.scheduleDateFilter = '';
        this.orderClearanceDateFilter = '';
        this.inspectionOrderFilter = '';
        this.vehicleTypeFilter = '';
        this.customerExpectedDateFilter = '';
    ...
    handleSBUChange(event) {
        this.sbuFilter = event.detail.Label;
        // console.log('this.sbuFilter', this.sbuFilter);
        // console.log('this.sbuFilter', event.detail.label);

    }
    handleDCChange(event) {
        // console.log('event.detail.selectedRecordId ',event.detail.selectedRecordId);
        this.distributionChannelFilter = event.detail.selectedRecordId;
    }
    handleREGChange(event) {
        this.regionFilter = event.detail.value;
    }
    handleDIVChange(event) {
        this.divisionFilter = event.detail.selectedRecordId;
        // console.log(' this.divisionFilter ', this.divisionFilter);
    }
    handleSTATEChange(event) {
        this.stateFilter = event.detail.value;
        console.log('state', this.stateFilter);
    }

    handlePlantSelection(event) {
        console.log('event.detail.selectedRecordId ', event.detail.selectedRecordId);
        this.dispatchLocationCodeFilter = event.detail.selectedRecordId;
    }
    handleOrderSelection(event) {
        this.salesOrderNoFilter = event.detail.selectedRecordId;
    }
    handleStatus(event) {
        this.soStatusFilter = event.detail.value;
    }
    handleCustomerSelection(event) {
        this.customerCodeFilter = event.detail.selectedRecordId;
        console.log('customerCodeFilter', this.customerCodeFilter);
    }
    handleMAT3Change(event) {
        this.materialGroup3Filter = event.detail.value;
        console.log('this.materialGroup3Filter', this.materialGroup3Filter);
        console.log('material', event.detail.Label);
    }
    handleProductSelection(event) {
        this.skuCodeFilter = event.detail.selectedRecordId;
    }

    handleVehicalType(event) {
        this.vehicleTypeFilter = event.detail.value;
    }

    handleSODate(event) {
        this.soDateFilter = event.detail.value;
    }

    handleSalesExecutive(event) {
        this.executiveFilter = event.detail.value;
    }

    handleCityChange(event) {
        this.cityFilter = event.detail.value;
        console.log('city', this.cityFilter);
    }

    handleScheduleDate(event) {
        this.scheduleDateFilter = event.detail.value;
        console.log('schedule Date', this.scheduleDateFilter);
    }
    handleClearanceDate(event) {
        this.orderClearanceDateFilter = event.detail.value;
    }

    handleDelDate(event) {
        this.customerExpectedDateFilter = event.detail.value;
    }

    handleInspectionChange(event) {
        this.inspectionOrderFilter = event.detail.checked;
    }

    @track ShipToaccountOptions = [];

    handleShipToAccountChange(event) {
        this.shipToPartyCodeFilter = event.detail.value;
    }
    ...
    f_getSOData() {
        ...
        getSOData({ woid: this.rId, orderRecordType: 'Export' })
            .then((result) => {
    ...
    searchButton() {
        ...
        this.moreCount = [];
        this.lessCount = [];
        this.blankPlanningReady = [];
        this.planningready = [];
        this.blankPlanningReady = [];
        this.planningready = []
        console.log('click', JSON.stringify(this.allClients.length))
        var filterValue = {
            SBU1__c: this.sbuFilter,
            Distribution_Channel__c: this.distributionChannelFilter,
            Division__c: this.divisionFilter,
            Plant_Code__c: this.dispatchLocationCodeFilter,
            // dispatchLocationName: this.dispatchLocationNameFilter,
            soStatus: this.soStatusFilter,
            Id: this.salesOrderNoFilter,
            EffectiveDate: this.soDateFilter,
            Sales_Executive__c: this.executiveFilter,
            AccountId: this.customerCodeFilter,
            Ship_to_Party__c: this.shipToPartyCodeFilter,
            // shipToPartyName: this.shipToPartyNameFilter,
            Sales_District__c: this.regionFilter,
            state: this.stateFilter,
            City__c: this.cityFilter,
            Material_Group_3__c: this.materialGroup3Filter,
            // materialFrieghtGroup: this.materialFrieghtGroupFilter,
            Product2Id: this.skuCodeFilter,
            // skuName: this.skuNameFilter,
            First_Date__c: this.scheduleDateFilter,
            Order_Clearance_Date__c: this.orderClearanceDateFilter,
            inspectionOrder: this.inspectionOrderFilter,
            Customer_Raj_Petro_Vehical__c: this.vehicleTypeFilter,
            // nddSku: this.nddSkuFilter,
            customerExpectedDate: this.customerExpectedDateFilter
        }
        ...
        this.getSearchSODataJS(filterValue);

        // getSOLineItems({
        //     // accountId:this.AccountId,

        //     pData: JSON.stringify(this.allClients),
        //     filters: JSON.stringify(filterValue)
        // }).then((data) => {
        //     // console.log("f_getSOLineItem data-->", JSON.stringify(data));
        //     // console.log('searchbutton--->>', JSON.parse(data));
        //     // console.log('pData--->>', JSON.stringify(this.recordList));
        //     this.setDelay();
        //     if (data) {
        //         let v_linelist = JSON.parse(JSON.stringify(data));

        //         this.filterValues(v_linelist);
        //     }
        // })
        //     .catch(error => {
        //         this.setDelay();
